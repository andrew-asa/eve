...
import com.asa.eve.structure.app.listener.KeyStateManager;
...
public class NativeHookListener implements NativeKeyListener {
    ...
    public void nativeKeyPressed(NativeKeyEvent nativeKeyEvent) {
        ...
        if (keyCode == NativeKeyEvent.VC_CONTROL) {
            ...
            LoggerFactory.getLogger().debug("press ctrl");
            return;
        } else if (keyCode == NativeKeyEvent.VC_ALT) {
            keyState.setAlt(true);
            LoggerFactory.getLogger().debug("press alt");
            return;
        } else if (keyCode == NativeKeyEvent.VC_META) {
            keyState.setMeta(true);
            LoggerFactory.getLogger().debug("press meta");
            return;
        } else if (keyCode == NativeKeyEvent.VC_SHIFT) {
            keyState.setShift(true);
            LoggerFactory.getLogger().debug("press shift");
            return;
        }
        int javaKeyCode = getJavaKeyCode(nativeKeyEvent);
        int mask = 0;
        ...
        GlobalKeyListenerManager.getInstance().keyPressed(keyStroke);
    }
    ...
    public void nativeKeyReleased(NativeKeyEvent nativeKeyEvent) {
        ...
        if (keyCode == NativeKeyEvent.VC_CONTROL) {
            ...
            LoggerFactory.getLogger().debug("Released ctrl");
            return;
        } else if (keyCode == NativeKeyEvent.VC_ALT) {
            keyState.setAlt(false);
            LoggerFactory.getLogger().debug("Released alt");
            return;
        } else if (keyCode == NativeKeyEvent.VC_META) {
            keyState.setMeta(false);
            LoggerFactory.getLogger().debug("Released meta");
            return;
        }else if (keyCode == NativeKeyEvent.VC_SHIFT) {
            keyState.setShift(false);
            LoggerFactory.getLogger().debug("Released shift");
            return;
        }
    }
